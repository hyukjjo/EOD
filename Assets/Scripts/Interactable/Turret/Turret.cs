...
public class Turret : MonsterTarget
{
    enum TurretState
    {
        Able = 0,
    ...
    [SerializeField] private Transform _mainTurret;
    ...
    private TurretState _state = TurretState.Able;
                    ...
                    if (TargetCollider.Length > 0)
                    {
                        ...
                        foreach (var target in TargetCollider)
                        {
                            ...
                            if (Vector3.Dot(direction.normalized, transform.forward) > GetAngle(_viewAngle / 2).z)
                            {
                                ...
                                Debug.DrawRay(transform.position, dirToTarget * 10f, Color.red, 2f);

                                if (Physics.Raycast(transform.position, dirToTarget, out RaycastHit hit, dstToTarget, ~_ignoreLayer))
                                {
                        ...
                        foreach (var target in _shotTargets)
                        {
                            if (!_fireTarget)
                            {
                                if(!target.GetComponent<Monster>().isDying)
                                    _fireTarget = target;
                                else
                                    continue;
                            }
                            ...
                            if (Distance < Vector3.Distance(transform.position, _fireTarget.position))
                            {
                                _fireTarget = target;
                            }
                        }
                        ...
                        if (_fireTarget)
                        {
                            _mainTurret.LookAt(_fireTarget.GetComponent<Monster>().GetHitPosition());
                            ...
                            if (_fireTarget.GetComponent<Monster>().isDying)
                            {
    ...
    private void Fire()
    {
        Vector3 shotDirection = Vector3.Slerp(_firePoints[_fireOrder].forward, Random.insideUnitSphere, _bulletSpreadAngle / 180f);
        ...
        bullet.transform.SetPositionAndRotation(_firePoints[_fireOrder].position, Quaternion.LookRotation(shotDirection));
        bullet.AddForce(bullet.transform.forward * _bulletSpeed, ForceMode.VelocityChange);

        if (_fireAudio[_fireOrder])
            _fireAudio[_fireOrder].PlayOneShot(_fireAudio[_fireOrder].clip);

        _fireOrder++;

        if (_fireOrder == _firePoints.Length - 1)
            _fireOrder = 0;
    }
    ...
    private void TargettingInsideMonsters()
    {
        if (_isActive)
        {
            ...
            {
                monster.CheckCloseTurret();
                //if (monster.GetMoveTarget() != transform)
